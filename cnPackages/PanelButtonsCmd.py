...
#		Label(self, text ="Cmds", font = "{courier new} 12 bold", bg = bg, fg = "blue").grid(row = 1, column = 1, columnspan = 2)
		...
		self.refIconPOMj = PhotoImage(file = './images/btnPOMj.gif')
		self.refIconPOMr = PhotoImage(file = './images/btnPOMr.gif')
...
#		self.btnPOM = Button(self, text = "POM", image = self.refIconPOMj, relief = GROOVE)						# Origine machine
		self.btnPOM = ButtonFlipFlop(self, text = "POM", imgon = self.refIconPOMj, imgoff = self.refIconPOMr, relief = GROOVE,
									 action = lambda arg = "POM": self.fctExt(arg))
...
#		c = 2
		...
		for btnRef in self.btnList:												# Cas des boutons de la class Button
			...
			if (btnCode not in ['POM', 'MAN', 'POZ', 'PXY']):					# Cas particulier des boutons de la class ButtonFlipFlop()
...
if __name__ == "__main__":
	...
	frm = Frame(w, bg = bg)
	frm.pack()
	btnStop = Button(frm, text = "Stop Clign", command = lambda arg = 'POM': pbc.stop(arg))
